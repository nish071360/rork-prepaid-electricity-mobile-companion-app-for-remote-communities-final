...
export function useAppState() {
  ...
  useEffect(() => {
    (async () => {
      ...
      // MOCK_CREDIT_TRANSACTIONS.forEach(async (transaction) => {
      ...
      balance && setState({ ...state, creditRemaining: balance });
    })();
    ...
    const intervalsPassedToday = Math.floor(
      (currentHour * 60 + currentMinute) / 15
    );

    const todayRecords = MOCK_RECORDS.slice(0, intervalsPassedToday);
    const todayUsage =
      todayRecords.reduce((sum, record) => sum + record.kwh_wh, 0) / 1000; // Convert to kWh
    ...
    switch (range) {
      case "day":
        ...
        return Array.from({ length: 24 }, (_, hour) => {
          ...
          return (
            hourRecords.reduce((sum, record) => sum + record.kwh_wh, 0) / 1000
          );
        });
